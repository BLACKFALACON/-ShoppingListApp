...
class _ShoppingListItemPageState extends State<ShoppingListItemPage> {
 ...
 @override
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: <Widget>[
       AppBar(title: Text("Alışveriş Listesi"),actions: <Widget>[IconButton(icon: Icon(Icons.done_all), onPressed: (){})],),
       Expanded(
         child:Stack(
          children: <Widget>[
          FutureBuilder(
           ...
           builder:(BuildContext context ,AsyncSnapshot<List<Item>> snapshot){
             if(snapshot.hasData && snapshot.data.length == 0){
               return Center(child: Text("Alışveriş listesi bulunamadı"));
              }
            if(snapshot.hasData){
             ...
             return ListView.builder(
              ...
              return GestureDetector(
               onLongPress:()async{
                bool result  = await  showDialog(context: context,builder:(BuildContext context)=>ConfirmDialog(item:item),);
